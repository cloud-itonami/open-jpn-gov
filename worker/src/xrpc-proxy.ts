...
// This file imports from @sveltejs/kit (json, RequestEvent, RequestHandler) and
// therefore will NOT run as-is now that the SvelteKit build is gone — there is no
// SvelteKit runtime in this repo to provide those imports or the $types module.
// It is preserved here byte-identical (below the header comments added by this
// migration) purely so the POST /xrpc/<nsid> -> AGENTGATEWAY_MCP_ROUTER_URL
// forwarding logic it implemented is not lost to history.
// Whether to revive this (e.g. rewritten as a plain fetch handler wired into
// worker/src/app.ts) is an undecided product question — not something this
// migration decides. Per README.md (verified 2026-08-15), the forwarding target
// (mcp.etzhayyim.com) has no DNS record either, so this was already non-functional
// as configured before this file was moved.
